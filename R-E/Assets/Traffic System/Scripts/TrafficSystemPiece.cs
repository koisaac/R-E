...
public class TrafficSystemPiece : MonoBehaviour
{
	...
	public  bool                               m_meshObjectAutoFindOnAwake    = true;                   // if this is true, the Awake function will try and find the child gameobject named "Models" since that is where all the rendering components are in this packages road pieces.
	...
	void Awake()
	{
		...
		{
			if (transform.GetChild (cIndex).name == "Models") 
			{
				m_meshObject = transform.GetChild (cIndex).gameObject;
				break;
			}
		}
	}
