...
public class TrafficSystemVehicle : MonoBehaviour 
{
	...
	public  GameObject                  m_meshObject           = null;                   // you need to assign the root gameobject that holds all the rendering components of the car. This is to turn this car on and off when culling is enabled. 
...
//				transform.position = nodeSearch.transform.position;
	...
	void FixedUpdate()
	{
		if (TrafficSystem.Instance && m_meshObject) 
		{
			...
			if (TrafficSystem.Instance.IsInView (transform.position, true))
